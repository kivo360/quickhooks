...
def test_official_format_compliance():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(f"   Exit Code: {result.returncode} ✅" if result.returncode == 0 else f"   Exit Code: {result.returncode} ❌")
        ...
        if result.stdout.strip():
            try:
                ...
                print(f"   - suppressOutput: {response.get('suppressOutput', 'missing')}")
                
                # Verify official fields
                if 'continue' in response and isinstance(response['continue'], bool):
                    print(f"   - 'continue' field: ✅ (bool)")
                else:
                    print(f"   - 'continue' field: ❌ (missing or wrong type)")
                    
            except json.JSONDecodeError:
                print(f"   JSON Response: ❌ Invalid JSON")
                print(f"   Raw stdout: {result.stdout}")
        else:
            if test_case['expect_json']:
                print(f"   JSON Response: ⚠️  None (expected some)")
            else:
                print(f"   JSON Response: ✅ None (expected)")
        ...
        if result.stderr.strip():
            ...
            print(f"   Stderr Feedback: No feedback")
...
def test_configuration_format():
    ...
    try:
        ...
        if isinstance(matcher, list) and len(matcher) > 0:
            ...
            print(f"   Tools: {', '.join(matcher[:5])}{'...' if len(matcher) > 5 else ''}")
        else:
        ...
        print(f"   - Format: JSON ✅")
        print(f"   - Location: ~/.claude/settings.json ✅")
        print(f"   - Structure: hooks.PreToolUse[] ✅")
        print(f"   - Matcher: Array of tool names ✅")
        print(f"   - Hook type: 'command' ✅")
...
def generate_compliance_report():
    ...
    compliance_items = [
        ...
        ("Official Hook Response Format", official_format_ok, "✅" if official_format_ok else "❌"),
        ("Current Implementation Functionality", current_impl_ok, "✅" if current_impl_ok else "❌"),
    ...
    for item, status, icon in compliance_items:
        print(f"{icon} {item}")
