...
from hooks.context_portal_memory import ContextPortalMemoryManager, ContextPortalHook
...
class TestContextPortalMemoryManager:
    ...
    def test_store_task(self):
        ...
        with sqlite3.connect(self.manager.db_path) as conn:
            cursor = conn.execute("SELECT * FROM tasks WHERE title LIKE ?", ("%authentication%",))
...
class TestContextPortalHook:
    ...
    def setup_method(self):
        """Set up test hook."""
        self.temp_dir = tempfile.mkdtemp()
        with patch('hooks.context_portal_memory.os.getcwd', return_value=self.temp_dir):
    ...
    def test_transform_with_decision_detection(self):
        ...
        result = self.hook.transform("Task", tool_input)
...
class TestHookIntegration:
    ...
    def setup_method(self):
        ...
        self.temp_dir = tempfile.mkdtemp()
        self.hook_script = Path(__file__).parent.parent / "hooks" / "context_portal_memory.py"
...
@pytest.mark.parametrize("tool_name,tool_input,expected_context", [
    ("Bash", {"command": "pytest tests/"}, {"tool": "Bash", "command": "pytest tests/"}),
    ("Edit", {"file_path": "test.py"}, {"tool": "Edit", "file_path": "test.py", "operation": "edit"}),
    ("Write", {"file_path": "new.py"}, {"tool": "Write", "file_path": "new.py", "operation": "write"}),
    ("Read", {"file_path": "readme.md"}, {"tool": "Read", "file_path": "readme.md", "operation": "read"}),
...
def test_parametrized_context_extraction(tool_name, tool_input, expected_context):
    ...
    try:
        with patch('hooks.context_portal_memory.os.getcwd', return_value=temp_dir):
