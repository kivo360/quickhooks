...
class TestHook(BaseHook):
    ...
    async def execute(self, input_data: HookInput, context: ExecutionContext) -> HookOutput:
        """Execute the test hook."""
        return HookOutput(
            status=HookStatus.SUCCESS,
            data={"message": "Test hook executed", "input_event": input_data.event_type},
...
class TestBaseHookExecution:
    ...
    async def test_hook_execution_timing(self):
        ...
        assert result.output_data.execution_time >= 0.1  # Should take at least 0.1 seconds
    ...
    async def test_hook_lifecycle_hooks(self):
        ...
        class LifecycleHook(BaseHook):
            async def before_execute(self, input_data: HookInput, context: ExecutionContext) -> None:
            ...
            async def after_execute(self, input_data: HookInput, context: ExecutionContext, result: HookOutput) -> None:
    ...
    async def test_hook_input_validation_in_run(self):
        ...
        invalid_input = HookInput(
            event_type="forbidden_event",
            data={}
        )
        
        context = ExecutionContext(
            hook_id="strict-hook-123",
    ...
    async def test_hook_exception_handling(self):
        ...
        hook = ExceptionHook()
        context = ExecutionContext(hook_id="exception-hook", execution_id="exec-exception")
...
class TestHookConcurrency:
    ...
    async def test_hook_cancellation(self):
        ...
        context = ExecutionContext(hook_id="cancellable-hook", execution_id="exec-cancel")
