...
from quickhooks.executor import ExecutionError, ExecutionResult, HookExecutor, PreToolUseInput
...
''')
    ...
    async def test_execution_timeout(self, executor, slow_hook_script):
        ...
        with pytest.raises(ExecutionError) as exc_info:
            await executor.execute(slow_hook_script, input_data.model_dump(), timeout=0.5)
    ...
    async def test_custom_timeout(self, executor, slow_hook_script):
        ...
        with pytest.raises(ExecutionError) as exc_info:
            await executor.execute(slow_hook_script, input_data.model_dump(), timeout=1.5)
...
''')
    ...
    async def test_realistic_hook_denied_tool(self, realistic_hook_script):
        ...
        input_data = PreToolUseInput(
            tool_name="rm",
            tool_input={"path": "/important/file.txt"}
        )
