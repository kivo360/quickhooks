...
def check_requirements() -> Dict[str, bool]:
    ...
    try:
        import quickhooks.agent_analysis
        checks['quickhooks_installed'] = True
    except ImportError:
...
def display_requirements_check(checks: Dict[str, bool]):
    ...
    requirements = [
        ...
        ("dependencies_installed", "Dependencies", "All required dependencies are installed")
    ]
...
def create_settings_json():
    ...
    confidence_threshold = float(Prompt.ask(
    ...
    settings_data['environment']['QUICKHOOKS_CONFIDENCE_THRESHOLD'] = str(confidence_threshold)
    ...
    hook_config = {
        "user-prompt-submit": {
            "script": str(Path.home() / '.quickhooks' / 'hooks' / 'agent_analysis_hook.py'),
...
@app.command()
def setup(
    skip_check: bool = typer.Option(False, "--skip-check", help="Skip requirements check"),
    ...
    console.print("1. Copy the hook configuration from ~/.quickhooks/claude_code_settings.json")
    ...
    console.print(f"   • Hook script: ~/.quickhooks/hooks/agent_analysis_hook.py")
    console.print(f"   • Settings: ~/.quickhooks/claude_code_settings.json")
    console.print(f"   • Sample agent: ~/.claude/agents/sample_coding_agent.py")
...
def uninstall():
    ...
    if not Confirm.ask("Are you sure you want to uninstall the QuickHooks Agent Analysis integration?"):
    ...
    console.print("Don't forget to remove the hook configuration from your Claude Code settings.json")
