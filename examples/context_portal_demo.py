...
from context_portal_memory import ContextPortalMemoryManager, ContextPortalHook
...
def demo_hook_integration():
    ...
    try:
        ...
        for scenario in tool_scenarios:
            ...
            should_process = hook.should_process(scenario['tool_name'], scenario['tool_input'])
            print(f"  Should process: {should_process}")
            
            if should_process:
                # Extract context information
                context_info = hook.extract_context_info(scenario['tool_name'], scenario['tool_input'])
                print(f"  Context extracted: {context_info['tool']} at {context_info['timestamp'][:19]}")
                ...
                if result:
                    print(f"  ✨ Tool input enhanced with context")
                    if '_context_portal_history' in result:
                        print(f"    Added {len(result['_context_portal_history'])} historical entries")
                else:
                    print(f"  📝 Context stored, no enhancement needed")
...
def demo_json_hook_interface():
    ...
    try:
        hook_script = Path(__file__).parent.parent / "hooks" / "context_portal_memory.py"
            ...
            if process.returncode == 0:
                try:
                    ...
                    if output.get('modified'):
                        print(f"    🔧 Tool input was enhanced with context")
...
def main():
    ...
    try:
        ...
        print("1. Copy hooks/context_portal_memory.py to your project's hooks/ directory")
