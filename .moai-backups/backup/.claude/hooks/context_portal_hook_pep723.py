...
class ContextPortalHook:
    ...
    def __init__(self):
        ...
        if self.verbose:
            print(
                f"[QuickHooks Context Portal] Initialized with db at {self.db_path}",
                file=sys.stderr,
            )
    ...
    def store_context(self, session_id: str, prompt: str, context: str = "") -> None:
        ...
        try:
            ...
            existing_context["prompts"].append(
                {
                    ...
                    "context": context,
                    "timestamp": str(
                        Path(context_file).stat().st_mtime
                        if context_file.exists()
                        else 0
                    ),
                }
            ...
            if self.verbose:
                print(
                    f"[QuickHooks Context Portal] Stored context for session {session_id}",
                    file=sys.stderr,
                )

        except Exception as e:
            print(
                f"[QuickHooks Context Portal] Error storing context: {e}",
                file=sys.stderr,
            )
    ...
    def retrieve_context(self, session_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if context_file.exists():
                ...
                if self.verbose:
                    print(
                        f"[QuickHooks Context Portal] Retrieved {len(context_data.get('prompts', []))} prompts for session {session_id}",
                        file=sys.stderr,
                    )
            ...
            print(
                f"[QuickHooks Context Portal] Error retrieving context: {e}",
                file=sys.stderr,
            )
            return {}
...
def process_hook_event(event_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if existing_context and existing_context.get("prompts"):
        ...
        if hook.verbose:
            response["message"] = (
                f"Retrieved {num_prompts} previous prompts from context"
            )
