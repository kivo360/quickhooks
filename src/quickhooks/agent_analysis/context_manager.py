...
class ContextManager:
    ...
    def _is_technical_text(self, text: str) -> bool:
        ...
        matches = sum(1 for word in technical_indicators if word.lower() in text.lower())
        return matches >= 3
    ...
    def chunk_context(self, context: str, prompt: str) -> List[ContextChunk]:
        ...
        available_tokens = self.effective_max - prompt_tokens - 500  # Reserve for response
    ...
    def _get_chunk_by_tokens(self, text: str, target_tokens: int, from_start: bool = True) -> str:
        """
        ...
        if from_start:
            ...
            for break_char in ['\n\n', '\n', '. ', '! ', '? ', '; ']:
                ...
                if first_break != -1 and first_break < target_chars * 0.3:  # Don't break too late
