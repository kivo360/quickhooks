...
class AgentDiscovery:
    ...
    def __init__(self, agents_dir: Optional[Path] = None, db_path: Optional[Path] = None):
        """
    ...
    def find_relevant_agents(self, prompt: str, context: str = "", limit: int = 5, min_similarity: float = 0.3) -> List[DiscoveredAgent]:
        """
    ...
    def _parse_agent_file(self, file_path: Path) -> Optional[DiscoveredAgent]:
        ...
        try:
            ...
            if file_path.suffix == '.py':
                description, capabilities, usage_pattern = self._parse_python_agent(content)
            elif file_path.suffix == '.md':
                description, capabilities, usage_pattern = self._parse_markdown_agent(content)
            elif file_path.suffix == '.json':
                description, capabilities, usage_pattern = self._parse_json_agent(content)
            else:
                # Generic text file
            ...
            if '"""' in stripped or "'''" in stripped:
                if not in_docstring:
                    ...
                    if stripped.count('"""') == 2 or stripped.count("'''") == 2:
                        content_part = stripped.split('"""')[1] if '"""' in stripped else stripped.split("'''")[1]
    ...
    def _extract_capabilities_from_text(self, text: str) -> List[str]:
        ...
        capability_keywords = [
            ...
            'research', 'investigate', 'find',
            'planning', 'design', 'architecture',
            'generation', 'create', 'build'
        ]
