...
class AgentRecommendation(BaseModel):
    ...
    priority: int = Field(ge=1, le=10, description="Priority ranking (1=highest, 10=lowest)")
...
class AgentAnalysisRequest(BaseModel):
    ...
    context: Optional[str] = Field(default=None, description="Additional context if available")
    max_context_tokens: int = Field(default=128000, description="Maximum context length in tokens")
...
class DiscoveredAgentInfo(BaseModel):
    ...
    similarity_score: float = Field(ge=0.0, le=1.0, description="Similarity score to the prompt")
...
class AgentAnalysisResponse(BaseModel):
    ...
    recommendations: List[AgentRecommendation] = Field(description="List of agent recommendations")
    discovered_agents: List[DiscoveredAgentInfo] = Field(default=[], description="Relevant agents found in filesystem")
    context_used: List[ContextChunk] = Field(description="Context chunks that were analyzed")
    ...
    multiple_agents_recommended: bool = Field(description="Whether multiple agents are recommended")
    claude_code_prompt_modification: Optional[str] = Field(default=None, description="Suggested prompt modification for Claude Code")
    
    @property
    def top_recommendation(self) -> Optional[AgentRecommendation]:
