...
class GrepToRipgrepTransformer:
    ...
    def parse_grep_command(self, command: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                'prefix': tokens[:grep_index],  # Commands before grep (e.g., sudo, env vars)
    ...
    def transform_args(self, args: List[str]) -> Tuple[List[str], List[str]]:
        ...
        while i < len(args):
            ...
            if arg.startswith('-'):
                ...
                if arg in ['-A', '--after-context', '-B', '--before-context', '-C', '--context']:
                    if i + 1 < len(args):
...
def main():
    ...
    try:
        ...
        if transformed_command and transformed_command != command:
            ...
            print(f"Grep→Ripgrep: {command[:50]}{'...' if len(command) > 50 else ''} → rg", file=sys.stderr)
