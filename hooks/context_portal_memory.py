#!/usr/bin/env python3
"""
QuickHook: Context Portal Memory Management

Integrates Context Portal MCP for project memory management in Claude Code.
Automatically captures context, stores decisions, and retrieves relevant information.
"""
...
class ContextPortalMemoryManager:
    ...
    def store_decision(self, title: str, description: str = "", decision: str = "", 
                      rationale: str = "", alternatives: str = "", tags: List[str] = None) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (title, description, decision, rationale, alternatives, tags_str, content_hash))
            return True
        except Exception as e:
            print(f"Error storing decision: {e}", file=sys.stderr)
            return False
    
    def store_task(self, title: str, description: str = "", status: str = "pending",
                   priority: str = "medium", context: str = "") -> bool:
        """Store a project task in the context portal."""
    ...
    def store_pattern(self, name: str, description: str = "", code_example: str = "",
        ...
        """Store a code pattern in the context portal."""
    ...
    def store_context_entry(self, tool_name: str, command: str, context: str = "",
                           result: str = "", session_id: str = "") -> bool:
        ...
        try:
            ...
            print(f"Error storing context entry: {e}", file=sys.stderr)
            return False
    ...
    def search_patterns(self, query: str, category: str = "", limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if category:
                    ...
                    cursor = conn.execute("""
                        SELECT * FROM patterns 
                        WHERE name LIKE ? OR description LIKE ? OR use_cases LIKE ?
                        ORDER BY timestamp DESC
                        LIMIT ?
                    """, (f"%{query}%", f"%{query}%", f"%{query}%", limit))
                return [dict(row) for row in cursor.fetchall()]
        except Exception as e:
            print(f"Error searching patterns: {e}", file=sys.stderr)
            return []
    
    def search_context(self, tool_name: str = "", query: str = "", limit: int = 10) -> List[Dict[str, Any]]:
...
class ContextPortalHook:
    """Hook that integrates Context Portal memory management with Claude Code."""
    ...
    def __init__(self):
        ...
        self.decision_keywords = [
            ...
            'pattern', 'approach', 'solution', 'fix', 'refactor'
        ]
    ...
    def extract_context_info(self, tool_name: str, tool_input: Dict[str, Any]) -> Dict[str, str]:
    ...
    def detect_decision_context(self, tool_input: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    def enhance_with_context(self, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'command' in tool_input:
            ...
            relevant_context = self.memory_manager.search_context(query=command, limit=3)
        elif 'file_path' in tool_input:
            file_path = tool_input['file_path']
            relevant_context = self.memory_manager.search_context(query=file_path, limit=3)
        else:
            return enhanced_input
        
        # Add context as metadata if found
        if relevant_context:
            enhanced_input['_context_portal_history'] = relevant_context[:2]  # Limit to avoid bloat
        
        return enhanced_input
    
    def transform(self, tool_name: str, tool_input: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Transform tool input by adding relevant context."""
        if not self.should_process(tool_name, tool_input):
            return None
        
        # Store context entry
        context_info = self.extract_context_info(tool_name, tool_input)
        ...
        enhanced_input = self.enhance_with_context(tool_input)
        
        # Return enhanced input if it was modified
...
def main():
    ...
    try:
        ...
        print(f"Context Portal: Processing {tool_name} in session {session_id}", file=sys.stderr)
        
        # Process with Context Portal hook
        hook = ContextPortalHook()
        hook.session_id = session_id  # Use actual session ID
        
        if hook.should_process(tool_name, tool_input):
            # Store context and check for transformations
            transformed = hook.transform(tool_name, tool_input)
            if transformed:
                # Tool input was enhanced with context - provide feedback
                print(f"Context Portal: Enhanced {tool_name} with context history", file=sys.stderr)
                ...
                # Context stored but no enhancement - proceed normally
                # No JSON response needed (exit code 0 is sufficient)
                pass
        else:
            # Tool not processed - proceed normally
            # No JSON response needed (exit code 0 is sufficient)
            pass
