...
try:
    ...
    if quickhooks_path.exists():
        ...
        print("QuickHooks not found. Please install QuickHooks with agent analysis dependencies.")
        sys.exit(1)
...
class AgentAnalysisHook:
    ...
    def __init__(self):
        ...
        self.enabled = os.getenv("QUICKHOOKS_AGENT_ANALYSIS_ENABLED", "true").lower() == "true"
        self.model = os.getenv("QUICKHOOKS_AGENT_MODEL", "qwen/qwen3-32b")
        self.confidence_threshold = float(os.getenv("QUICKHOOKS_CONFIDENCE_THRESHOLD", "0.7"))
    ...
    def should_analyze(self, user_input: str) -> bool:
        ...
        if "agent" in user_input.lower() and ("use" in user_input.lower() or "with" in user_input.lower()):
            return False
        
        return True
    
    def analyze_and_modify_prompt(self, user_input: str, context: str = "") -> Dict[str, Any]:
        """
...
def on_user_prompt_submit(event_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if os.getenv("QUICKHOOKS_VERBOSE", "false").lower() == "true":
        print(f"[QuickHooks Agent Analysis]")
    ...
    if result["modified"]:
        ...
        event_data["metadata"]["quickhooks_agent_analysis"] = {
            ...
            "modified": True,
            "analysis_summary": result["analysis"]["analysis_summary"] if result["analysis"] else None,
            "discovered_agents": result["analysis"]["discovered_agents"] if result["analysis"] else [],
...
def main():
    ...
    for prompt in test_prompts:
        ...
        if result['analysis'] and result['analysis']['discovered_agents']:
            print(f"  Discovered {len(result['analysis']['discovered_agents'])} agents:")
