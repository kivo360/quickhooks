        ...
        try:
            ...
            for i, json_block in enumerate(json_blocks):
                try:
                    ...
                    print(f"   ⚠️  JSON block {i + 1}: Invalid JSON (may be example fragment)")
    ...
    for test_file in test_files:
        ...
        try:
            ...
            for pattern in dict_patterns:
                ...
                if matches:
                    print(f"   ⚠️  Found potential configuration patterns: {len(matches)}")
                    
            # Look for specific schema violations (but exclude this audit file itself)
            if '"matcher": [' in content and 'test_schema_compliance_audit.py' not in str(test_file):
                issues.append({
                    'file': str(test_file),
                    'issue': 'Uses old array format for matcher (should be string with | separator)'
                })
                print(f"   ❌ Uses old array format for matcher")
            ...
            if re.search(hook_desc_pattern, content, re.DOTALL):
                ...
                print(f"   ❌ Hook command includes invalid description field")
...
def audit_hook_implementations():
    ...
    for hook_file in hook_files:
        ...
        try:
            ...
            required_fields = ['session_id', 'tool_name', 'tool_input', 'hook_event_name']
            missing_fields = []
            ...
            if missing_fields:
                ...
                print(f"   ✅ Handles all required input fields")
            ...
            has_valid_output = any(re.search(pattern, content) for pattern in valid_output_patterns)
            
            if has_valid_output:
                print(f"   ✅ Uses valid output format")
            else:
                issues.append({
                    'file': str(hook_file),
                    'issue': 'May not use correct output format (JSON with continue/suppressOutput or exit code)'
                })
                print(f"   ⚠️  Output format may need verification")
...
def audit_generated_configurations():
    ...
    for scenario in test_scenarios:
        ...
        try:
            ...
            if is_valid:
                print(f"   ✅ Generated valid configuration")
                ...
                print(f"   ❌ Generated invalid configuration:")
                for error in errors:
                    print(f"      • {error}")
...
def create_schema_compliance_report():
    ...
    overall_status = "✅ FULLY COMPLIANT" if total_issues == 0 else f"⚠️  {total_issues} ISSUES FOUND"
